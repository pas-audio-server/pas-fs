...
bool ParseOptions(int argc, char * argv[], bool & real_db, string & path, string & nspace, bool & do_checksum_analysis, string & dbhost)
{
	bool rv = false;
	int opt;
	...
	while ((opt = getopt(argc, argv, "cn:dp:h:")) != -1) {
		switch (opt) {
			/*
			case 'c':
...
int main(int argc, char * argv[])
{
	...
	LOG2(_log_, "", FATAL);
	try {
		if (ParseOptions(argc, argv, do_db, root, nspace, do_checksum_analysis, dbhost)) {
			if (*(root.end() - 1) == '/')
